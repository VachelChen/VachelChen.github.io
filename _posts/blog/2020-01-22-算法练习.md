...
Dijkstra算法是可以理解为BFS类问题+有权值的边的问题，此时可以考虑用优先队列：  
Priority Queue.  
...
//访问
//优先队列不存在front() 与 back()函数，只能通过top()函数来访问
q.top();
...
//优先级设置
//默认数字越大的优先级越高，char就是字典序最大的那个
priority_queue<int,vector<char>,less<int>> q;
